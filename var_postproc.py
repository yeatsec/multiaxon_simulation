...
for temp_index in range(num_temps):
	...
	for sd_index in range(sig_size):
		sd_cap_at_temp[temp_index][i] = t_alpha_df*(sd_cap_at_temp[temp_index][i]/num_pops)**(0.5)

# plot it all
plt.figure()
plt.title("Temperature Dependent CAP Evolution", fontsize=22)
plt.ylabel("Extracellular Voltage (mV)", fontsize=22)
plt.xlabel("Time (ms)", fontsize=22)
ax = plt.gca()
for temp_index in range(num_temps):
	plt.plot(timesteps, mean_cap_at_temp[temp_index], color=colors[temp_index])
	ax.fill_between(timesteps, [mean_cap_at_temp[temp_index][i] + sd_cap_at_temp[temp_index][i] for i in range(sig_size)], [mean_cap_at_temp[temp_index][i] - sd_cap_at_temp[temp_index][i] for i in range(sig_size)], color=colors[temp_index], alpha=0.5)
plt.show()